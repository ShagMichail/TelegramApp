...
//
//  ProfileGalleryControllerNode.swift
//  divo-ios
//
//  Created by Michail Shagovitov on 24.03.2026.
//
...
final class ProfileGalleryControllerNode: ASDisplayNode {
    ...
    func updateData(photos:[UserPhoto], videos: [UserVideoItem]) {
        ...
        let addedToStartCount = newCount - oldCount
        
        if newCount > oldCount {
            self.mainCollectionView.reloadData()
            self.previewCollectionView.reloadData()
            
            if previousIndex == 0 && addedToStartCount == 1 {
                self._currentIndex = 0
            } else {
                self._currentIndex = previousIndex + addedToStartCount
            }
            
            self.mainCollectionView.layoutIfNeeded()
            let mainOffset = CGPoint(x: CGFloat(self._currentIndex) * self.mainCollectionView.bounds.width, y: 0)
            self.mainCollectionView.setContentOffset(mainOffset, animated: false)
