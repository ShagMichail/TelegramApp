...
final class EditSocialLinksNode: ASDisplayNode, UITextFieldDelegate {
    ...
    init(context: AccountContext, presentationData: PresentationData, userDetailData: UserDetail?, addPhoto: @escaping () -> Void) {
...
//        let websiteValue = userDetailData?.userSocialNetworks?[0].url
//        
//        if let urlString = userDetailData?.userSocialNetworks?[1].url, let url = URL(string: urlString) {
//            instagramName = url.pathComponents.last(where: { $0 != "/" }) ?? ""
//        }
//        if let urlString = userDetailData?.userSocialNetworks?[2].url, let url = URL(string: urlString) {
//            tiktokName = url.pathComponents.last(where: { $0 != "/" }) ?? ""
//        }
//        if let urlString = userDetailData?.userSocialNetworks?[3].url, let url = URL(string: urlString) {
...
//        }
        ...
        self.websiteTextField = DivoTextField(title: websiteValue, prefix: "")
