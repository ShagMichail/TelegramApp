    ...
    |> mapToSignal { result -> Signal<Int64?, NoError> in
        ...
        case let .inputFile(inputFile):
            return account.network.request(Api.functions.messages.uploadMedia(
                ...
                peer: .inputPeerEmpty,
                media: Api.InputMedia.inputMediaUploadedPhoto(.init(flags: 0, file: inputFile, stickers: nil, ttlSeconds: nil))
            ))
            |> map { (response: Api.MessageMedia) -> Int64? in
                switch response {
                case let .messageMediaPhoto(messageMediaPhotoData):
                let (_, photo, _) = (messageMediaPhotoData.flags, messageMediaPhotoData.photo, messageMediaPhotoData.ttlSeconds)
                    if let photo = photo {
                        switch photo {
                        case let .photo(photoData):
                            return photoData.id
                        case let .photoEmpty(photoData):
                            return photoData.id
                        }
                    }
                default:
